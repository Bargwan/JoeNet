...
Contains the deterministic mathematical logic required for Potential-Based Reward Shaping. It houses a stateless engine that calculates the exact Expected Value (EV) in point differentials of any given board state, balancing the mathematical progression of completing a hand against the imminent threat of an opponent ending the round.
...
### 5.1 The Potential Function: Probabilistic EV
The Potential Function $\Phi(s)$ is strictly defined as the output of the mathematical engine located in `reward.py`.
Instead of abstract heuristics, this engine calculates the exact Expected Value of the hand state in point differentials, natively balancing two timelines:
* **Progression (Future Trajectory):** The combinatorial probability of completing the round objectives and catching opponents with deadwood.
* **Survival (Imminent Threat):** The probability of an opponent abruptly ending the round, resulting in a penalty based on the agent's current deadwood.

Because $\Phi(s)$ outputs an Expected Value in points, a positive dense reward ($R_t > 0$) mathematically proves the agent's action increased its point expectancy, providing a flawless gradient for the PyTorch Actor.

### 5.2 Dynamic Zero-Sum Conversion
Because Joe is a multiplayer free-for-all, the raw EV from the probabilistic engine must be normalized for the Critic. The engine calculates pairwise EV (Expected Reward - Expected Penalty Risk). For PBRS, this value is directly mapped as the baseline State Value.
...
* **Targets:**
    * **Critic Target:** The Critic bootstraps its own predictions step-by-step using the dense Expected Value ($\Delta EV$) generated by the mathematical engine in `reward.py`, anchored by the terminal score at the exact end of the round.
